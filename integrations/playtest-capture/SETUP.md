...
> **⚠️ SessionRoot 是全机全局键(多项目必看)**:窗口里若设了「标注产物根目录」(EditorPrefs `Yide.Playtest.SessionRoot`),它**跨所有 Unity 工程共用一个值**。一旦某项目指到共享目录(如 Google Drive),**所有项目**按 F8 都会写进那里、混在一起。**一台机器跑多个项目时**:每个项目要么把它**留空**(走各自工程内 `QA/playtest`,自动隔离,推荐),要么**各指一个独立目录**,别共用。翼德的 `playtest.js` 已加护栏:发现一场混入多个来源项目会告警,但最干净还是源头不共用。
