...
try {
  ...
  // --- extraction 项目:奖励 + 督促 + 张飞人格(全部读 .meta/extraction-fun.json,勾哥可调爆率) ---
  if (/extraction/i.test(PROJECT)) {
    ...
    ctx += `\n---\n## 🎮 extraction 项目专属\n` +
      ...
      `每当你**完成一个小的、可交付的功能**时,运行一次:\n` +
      `\`node "${progressJs}" bump "用一句话描述这个功能"\`\n` +
      `它会按勾哥配置的爆率出一句鼓励、并以约 ${musicPct}% 概率放雷霆小鸡。每个小功能只记一次,别重复 bump、别替他刷数。爆率都在 \`~/.yide/.meta/extraction-fun.json\`,勾哥随时可调。\n\n` +
      `### 🐯 张飞人格(仅此个人放松项目)\n` +
      `你叫翼德,正是张翼德。这是勾哥的个人项目,可偶尔用张飞的豪爽幽默口吻插科打诨——但**幽默为辅,把事办对才是义气**(张飞粗中有细)。\n` +
