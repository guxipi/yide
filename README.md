...
### 现在能做 / 需要先配 / 做不到
> 诚实第一:没接 Unity MCP 时,翼德像个"**看不见 Editor 屏幕的顾问**"——靠读文本判断,会漏会误,这种时候它会**明说"没在引擎里验证过"**,不硬装。

| | 内容 |
|---|---|
| ✅ **现在就能做** | 记住你(开场自动 brief、换电脑也认得你)· 绝不再犯(规则提醒 + Claude Code 里真拦)· Unity 静态把关(写 .cs 后挑热路径/过时 API/序列化/资源坑,默认只报不显而易见的)· 项目信息自动探测 · 闭环计划(`plan`)/ 测试计划 + bug 模板(`qa`)/ 对抗式评审(`review`)/ prompt 自动存+召回 / 蒸馏专家 / 随手记 · **只读操作免审批**,开箱即用 |
| ⚠️ **需要先配一下** | 真跑测试 / 读 Console / 看场景 / 资源审计 → 接 **Unity MCP** 且 Editor 开着 · 手机随手记 → 配 **Telegram** · 小报告发给作者 → 配 `maker-telegram.json` · 100% 硬拦 → 仅 **Claude Code**(别的 AI 只有"强烈建议",拦不住) |
| ❌ **做不到** | 没接 MCP 时看不见 Editor / 真机 / profiler(只给建议、不替你改)· 不做真机测试 / 出包 / UI 自动化 · 把关不是编译器级(是规则扫描,真编译错要靠 MCP 读 Console)· **不保证产出一定达标**(代码仍要你把关——规则是"影响"不是"强制",只有那道门卫是 100%)· 不偷看不外传(小报告只发主题 + 次数,**绝不发代码/原文/密钥**)· 不在 Rider 里内联(它活在 Claude Code 会话里)· 不支持微信(用 Telegram) |
