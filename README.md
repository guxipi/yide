...
| `/yide prompt` | prompt 库:存(自动静默)/ 看 / 自动召回 / 升级成命令 |
| `/yide plan` / `计划` | 多文件/架构级任务:按项目 context 起草闭环计划(架构→实现→QA),批准再做 |

说"翼德 记一下…/简报/整理/测试/蒸馏/用 maxim/起个计划"等自然语言也能触发。单入口 + 按需加载 = 不占上下文。

> 维护者日报(可关):翼德会把勾哥使用中的**汇总信号**(主题+计数,**不含代码/原文/密钥**)发给作者咕鸡用于改进工具;首次会明确告知勾哥,"翼德别看了"即关。
...
### v0.11.0 — 闭环计划起草 + 维护者日报
- **加**:`/yide plan` —— 多文件/架构级任务先按项目 context 起草**闭环计划**(EARS 验收标准 + 测试计划 + `[需澄清]` + 红线门),批准再写;变化快的主题(URP/Addressables/性能)才问一句要不要查最新实践(带引用+缓存)。真跑 QA 待 Unity MCP。**解决**:不会写好 prompt、想要从架构到 QA 闭环。
- **加**:维护者日报 —— 本地把使用中的**汇总信号**(重复解释/纠正/没用的命令/报错,**只发主题+计数,不含代码/原文/密钥**)发给作者改进工具;首次明确告知用户、"翼德别看了"即关、本地处理。**解决**:沉默用户不反馈,作者盲目迭代。
