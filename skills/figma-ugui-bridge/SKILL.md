...
description: Turn a Figma design into a Unity UGUI prefab using the self-built figma-ugui-bridge tool — NOT Figma MCP; multi-resolution anchors inferred. A THIN handbook; tool code lives in its own repo guxipi/figma-ugui-bridge. Trigger on "照figma摆" / "figma落地" / "从figma导UI" / "figma导ugui" / "figma到unity" / "figma2unity" / "配置figma2unity" / "我要配置 Figma2Unity" / 把 Figma 设计稿导成 Unity prefab. NOT for engine-direct UI work — write-MCP projects (e.g. Extraction) that say "摆UI/对齐" go to ui-placement instead.
---
