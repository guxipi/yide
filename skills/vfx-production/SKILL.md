---
name: vfx-production
description: Build production-grade ("成品级") 3D/world-space VFX in Extraction — drops, pickups, auras, bursts, on-character/projectile/impact FX; the world-particles/meshes/shaders/motion counterpart to ui-visual-rework (screen-space uGUI). Trigger on "做XX视效/特效" / "vfx 太cheap" / "重做掉落/拾取/光效" / "成品级 vfx" / "加个爆开/光环/拖尾/命中特效" / "把 EpicToon 用起来" / "3D 视觉重做" or any request to author/polish in-world visual effects. NOT for fixing an imported model's OWN look (grey/material/pivot/rig) — that's gaoguang-3d.
---
