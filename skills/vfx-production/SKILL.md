---
name: vfx-production
description: Build production-quality ("成品级") 3D / world-space VFX in Extraction — drops, pickups, auras, bursts, on-character effects, projectile/impact FX — to shippable premium-mobile quality. The VFX counterpart to ui-visual-rework (that one = screen-space uGUI; THIS one = world particles/meshes/shaders/motion). Trigger on "做XX视效/特效" / "vfx 太cheap" / "重做掉落/拾取/光效" / "成品级 vfx" / "加个爆开/光环/拖尾/命中特效" / "把 EpicToon 用起来" / "3D 视觉重做" or any request to author or polish in-world visual effects. Covers: asset-reuse-vs-custom decision, the runtime color-tint pipeline (and its alpha trap), pooling, the hover/pop/breath/burst motion language, on-player feedback (body glow + world→screen rising icons), and the MCP edit-mode-then-play-mode visual iteration loop. Battle-tested on the in-run loot drop + pickup overhaul (bubble shell + inner glow + collect splash + player glow + rising icons).
---
